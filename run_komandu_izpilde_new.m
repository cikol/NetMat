function result_all=run_komandu_izpilde_new(alpha,paths,N,gain,system,name)
%% environment
        ...
        for scale=1:length(noise_scale_vector)
            ...
            for Cs_i=1:length(Cs_vector)
                ...
                if strcmp(matlab,'yes')
                    ...
                    [mode,counts]=cs_test5_optimal_v2(Net,con,P_cst);
                    [mode_max_1path]=cs_test5_optimal_v2(Net,con(con(:,3)==1,:),Inf);
                    schemes=size(mode,2);
                    ...
                    for k=1:size(mode,2)
                        [rates(p:p+(size(mode{k},1))-1,:), R(:,k)]=rate_matrix3(Net,mode{k},'perfect',antena_type,paths,[],ic_scheme,w,ic_strat);
                        p=p+size(mode{k},1);
                    end
                    ...
                    for k_max=1:size(mode_max_1path,2)
                        [rates_max(p:p+(size(mode_max_1path{k_max},1))-1,:), R_max(:,k_max)]=rate_matrix3(Net,mode_max_1path{k_max},'perfect','ideal',paths,[]);
